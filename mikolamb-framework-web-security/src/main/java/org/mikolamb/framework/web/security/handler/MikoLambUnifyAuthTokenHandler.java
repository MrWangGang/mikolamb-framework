 ...
 **/
@Component
public class MikoLambUnifyAuthTokenHandler {
    @Resource
    ...
    public String setPrincipalByToken(String principalSign,String principal,String principalModel){
        if(StringUtils.isBlank(principalSign)){
            throw new MikoLambEventException(EA00000005);
        }
        if(StringUtils.isBlank(principal)){
            throw new MikoLambEventException(EA00000002);
        }
        if(principalModel == null){
            throw new MikoLambEventException(EA00000006);
        }
